...
var AuthorizationCodeClient = class extends BaseClient {
  ...
  async createAuthCodeUrlQueryString(request) {
    ...
    if (request.domainHint) {
      ...
      (_c = this.performanceClient) == null ? void 0 : _c.addFields({ domainHintFromRequest: true }, correlationId);
    }
    (_d = this.performanceClient) == null ? void 0 : _d.addFields({ prompt: request.prompt }, correlationId);
    if (request.prompt !== PromptValue.SELECT_ACCOUNT) {
      if (request.sid && request.prompt === PromptValue.NONE) {
        this.logger.verbose("createAuthCodeUrlQueryString: Prompt is none, adding sid from request");
        parameterBuilder.addSid(request.sid);
        (_e = this.performanceClient) == null ? void 0 : _e.addFields({ sidFromRequest: true }, correlationId);
        ...
        if (accountLoginHintClaim) {
          ...
          (_f = this.performanceClient) == null ? void 0 : _f.addFields({ loginHintFromClaim: true }, correlationId);
          try {
          ...
          (_g = this.performanceClient) == null ? void 0 : _g.addFields({ sidFromClaim: true }, correlationId);
          try {
            const clientInfo = buildClientInfoFromHomeAccountId(request.account.homeAccountId);
            parameterBuilder.addCcsOid(clientInfo);
          } catch (e) {
          ...
          (_h = this.performanceClient) == null ? void 0 : _h.addFields({ loginHintFromRequest: true }, correlationId);
        } else if (request.account.username) {
          this.logger.verbose("createAuthCodeUrlQueryString: Adding login_hint from account");
          parameterBuilder.addLoginHint(request.account.username);
          (_i = this.performanceClient) == null ? void 0 : _i.addFields({ loginHintFromUpn: true }, correlationId);
          try {
            const clientInfo = buildClientInfoFromHomeAccountId(request.account.homeAccountId);
            parameterBuilder.addCcsOid(clientInfo);
          } catch (e) {
        ...
        parameterBuilder.addLoginHint(request.loginHint);
        parameterBuilder.addCcsUpn(request.loginHint);
        (_j = this.performanceClient) == null ? void 0 : _j.addFields({ loginHintFromRequest: true }, correlationId);
      }
    } else {
...
var StandardController = class _StandardController {
  ...
  async handleRedirectPromiseInternal(hash) {
    ...
    const useNative = request && NativeMessageHandler.isPlatformBrokerAvailable(this.config, this.logger, this.nativeExtensionProvider) && this.nativeExtensionProvider && !hash;
...
function getPerfMeasurementModule() {
  ...
  try {
    ...
    if (Number(perfEnabled) === 1) {
      return import("./BrowserPerformanceMeasurement-TOSAPQ46.js");
    }
  } catch (e) {
  }
  return void 0;
